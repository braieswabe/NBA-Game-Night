...
function buildSimulation(args: {
  ...
  const simulation: Simulation = {
    ...
    imagePrompts: {
      ...
      championshipCelebration: `Championship still: winning fictional team ${winner.name} in custom ${winner.color} (no real NBA logos)—the finals MVP ${mvpPlayer} at center podium with the gold trophy, flanked by ALL FIVE starters in matching ${winner.name} jerseys so the full roster is visible: ${winnerPlayers.join(", ")}. Same franchise, one celebration huddle on the podium. Defeated fictional team ${loser.name} in ${loser.color} jerseys appears as a SECOND GROUP in soft-focus mid-ground or tunnel: the ENTIRE losing roster (${loserPlayers.join(", ")}) together, openly crying, tears, hugging or slumped, emotional collapse—clearly the losing side, never on the winner podium. Widescreen broadcast photography, dramatic arena lights.`,
    },
  };
